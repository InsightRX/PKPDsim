...
assert("1cmt oral ss correctly number of cmts", attr(PKPDsim:::advan_funcs[["1cmt_oral"]], "cmt") == 2)
...
res_ode <- sim(pk_1cmt_iv, parameters = par, regimen = reg_bolus, t_obs = t_obs, only_obs = F)$y
assert("2cmt oral ss correct number of cmts", attr(PKPDsim:::advan_funcs[["1cmt_iv_bolus"]], "cmt") == 1)
assert("1cmt iv bolus ss correct", max(delta(res_ana[1], res_ode[1])) < 0.001)

## 1 cmt iv infusion
# res_ana <- calc_ss_analytic(f = "1cmt_iv_infusion", dose = dose, interval = interval, parameters = par, )
# res_ode <- sim(pk_1cmt_iv, parameters = par, regimen = reg_inf, t_obs = t_obs, only_obs = F)$y
# assert("1cmt iv infusion ss correct", max(delta(res_ana[1:2], res_ode[1:2])) < 0.001)
# assert("1cmt infusion ss correct", max(delta(res_ana[1], res_ode[1])) < 0.001)
...
assert("2cmt oral ss correct number of cmts", attr(PKPDsim:::advan_funcs[["2cmt_oral"]], "cmt") == 3)
...
assert("2cmt iv bolus correct number of cmts", attr(PKPDsim:::advan_funcs[["2cmt_iv_bolus"]], "cmt") == 2)
...
assert("3cmt oral ss correct number of cmts", attr(PKPDsim:::advan_funcs[["3cmt_oral"]], "cmt") == 4)
...
assert("3cmt iv bolus correct number of cmts", attr(PKPDsim:::advan_funcs[["3cmt_iv_bolus"]], "cmt") == 3)
