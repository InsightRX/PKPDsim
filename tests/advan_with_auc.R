...
assert("AUC matches ODE sim", all(rdelta(res1_iv_c[res1_iv_c$TIME %in% t_obs,]$AUC, res1_iv_ode[res1_iv_ode$comp == 2,]$y) < 1e-5))
...
assert("AUC matches ODE sim", all(rdelta(res1_inf_c[res1_inf_c$TIME %in% t_obs,]$AUC, res1_inf_ode[res1_inf_ode$comp == 2,]$y) < 1e-5))
...
assert("AUC matches ODE sim", all(rdelta(res2_iv_c[res2_iv_c$TIME %in% t_obs,]$AUC, res2_iv_ode[res2_iv_ode$comp == 2,]$y) < 1e-5))
...
assert("AUC matches ODE sim", all(rdelta(res2_inf_c[res2_inf_c$TIME %in% t_obs,]$AUC, res2_inf_ode[res2_inf_ode$comp == 2,]$y) < 1e-5))
