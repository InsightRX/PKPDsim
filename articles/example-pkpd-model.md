...
the `A_init` argument, but this will not take any parameters into
account. However, we can also define initial states dynamically using
the `state_init` argument, which allow you to specify the state of each
...
![](example-pkpd-model_files/figure-html/example-pkpd-1.png)
...
models. In `PKPDsim` you can specify two or more model parts separately
