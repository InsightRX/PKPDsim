...
`new_ode_model` is the function that creates a new ODE model that can be
used in the
[`sim()`](https://insightrx.github.io/PKPDsim/reference/sim.md) command.
It defines the ODE system and sets some attributes for the model. The
model can be specified in three different ways:

- `model`: a string that references a model from the library included in
  `PKPDsim`. Examples in the current library are e.g. `pk_1cmt_oral`,
  `pk_2cmt_iv`. To show the available models, run
  [`new_ode_model()`](https://insightrx.github.io/PKPDsim/reference/new_ode_model.md)
  without any arguments.
- `code`: using code specifying derivatives for ODE specified in
  *pseudo-R* code
...
- array indices for the derivatives and compartments are indicated with
  `[ ]`. Compartments indices can start at either 0 or 1. If the latter,
  all indices will be reduced by 1 in the translation to C++ code.
- equations are defined using `=`
- power functions need to be written out as `pow(base,exp)`.
- force any numbers to be interpreted as `real`, to avoid them being
  interpreted as `integer`. So if an equation involves the real number
  `3`, it is usually safer to write this as `3.0` in code.

The input code is translated into a C++ function. You can check that the
model compiled correctly by typing the model name on the R command line,
...
If you’re interested, you can also output the actual C++ function that
is compiled by specifying the `cpp_show_code=TRUE` argument to the
[`new_ode_model()`](https://insightrx.github.io/PKPDsim/reference/new_ode_model.md)
function.
...
Also, when you want to use covariates in your ODE system (more info on
how to define covariates is in the *Covariates* vignette), you will have
...
functions. `PKPDsim` does not translate those from the *pseudo-R* code
to valid C++ syntax automatically. C/C++ does not use the `^` to
indicate power functions, but uses the `pow(value, base)` function
...
using the `dose` argument. By default, the dose will go into compartment
`1`, with a bioavailability of `1`. The `bioav` element in the list can
...
Bioavailability can also be used for dosing based on mg/kg, since that
is not supported in
[`new_regimen()`](https://insightrx.github.io/PKPDsim/reference/new_regimen.md).
The way to implement this is by scaling the dose by the “weight”
...
The observation compartment can be set by specifying a list to the `obs`
...
The `scale` can be either a parameter or a number, the `cmt` can only be
a number.

*Note that the variables specified inside the differential equation
block are not available as scaling parameters. E.g. for allometry you
...
Using the `file` argument, the model code is read from the specified
files. This is just a convenience function, i.e. it allows you to
